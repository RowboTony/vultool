...
func TestGetSupportedChains(t *testing.T) {
	...
	// Test that we have a reasonable number of supported chains
	if len(chains) < 10 {
		t.Errorf("Expected at least 10 chains, got %d", len(chains))
	}

	// Test that key chains are present
	mustHaveChains := []SupportedChain{ChainBitcoin, ChainEthereum, ChainSolana, ChainSUI}
	chainMap := make(map[SupportedChain]bool)
	for _, chain := range chains {
		chainMap[chain] = true
	}

	for _, expectedChain := range mustHaveChains {
		if !chainMap[expectedChain] {
			t.Errorf("Missing expected chain: %s", expectedChain)
		}
	}
}
